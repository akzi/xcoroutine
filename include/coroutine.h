...
struct Routine
{
	...
	Routine(const std::function<void()> & f)
	{
		func = std::move(f);
		fiber = nullptr;
	}
...
struct Ordinator
{
	std::vector<Routine *> suspended_routines;
	std::list<Routine *> routine_cache_;
	...
	~Ordinator()
	{
		for (auto &routine : suspended_routines)
			delete routine;
	}
};
...
inline routine_t create(Func&& f)
{
	...
	if (ordinator.routine_cache_.size())
	{
		routine = ordinator.routine_cache_.front();
		ordinator.routine_cache_.pop_front();
		routine->func = std::forward<Func>(f);
	}
	else
		routine = new Routine(std::forward<Func>(f));

	if (ordinator.indexes.empty())
	{
		ordinator.suspended_routines.push_back(routine);
		return ordinator.suspended_routines.size();
	}
	else
	{
		routine_t id = ordinator.indexes.front();
		ordinator.indexes.pop_front();
		assert(ordinator.suspended_routines[id-1] == nullptr);
		ordinator.suspended_routines[id-1] = routine;
		return id;
	}
}
...
{
	Routine *routine = ordinator.suspended_routines[id-1];
	assert(routine != nullptr);

	delete routine;
	ordinator.suspended_routines[id-1] = nullptr;
...
inline void __stdcall entry(LPVOID lpParameter)
{
	do
	{
		...
		routine->func();
		ordinator.current = 0;
		ordinator.routine_cache_.push_back(routine);
		ordinator.indexes.push_back(id);
		ordinator.suspended_routines[id - 1] = nullptr;
...
inline void yield(std::function<void()> &resume_func)
{
	routine_t id = ordinator.current;
	Routine *routine = ordinator.suspended_routines[id - 1];
	assert(routine != nullptr);
